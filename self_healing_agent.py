...
def extract_features(golden, candidate):
    """
    Extract binary features for matching:
      - f0: id match
      - f1: name match
      - f2: data-testid match
      - f3: class match
      - f4: text containment (1 if candidate text contains golden text)
      - f5: innerHTML containment (if applicable)
      - f6: normalized heuristic similarity score
    ...
    # Max heuristic score: primary (10+10+8+5+3+2=38) + parent (2+5+3=10) = 48.
    f6 = raw_score / 48.0
...
def self_heal_selector(driver, golden, training_data, training_labels, model):
    ...
    # Append current examples to training data
...
def get_updated_locator(driver, original_xpath, golden, training_data, training_labels, model):
    try:
        ...
        # Return 5 values consistently; candidate is None here.
...
class SelfHealingAgent:
    ...
    def locate_element(self, original_xpath):
        ...
        try:
            ...
            # Unpack consistently (ignoring the candidate returned by self-healing)
